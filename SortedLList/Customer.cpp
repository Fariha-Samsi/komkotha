...
bool Customer::hasNoPhoneNumber()
{
...
bool Customer::operator < (const Customer &customer) const
{
    ...
    int minimumLength = min(this->name.size(), customer.name.size());

    for (int i = 0; i < minimumLength; i++)
    {
        if (this->name[i] > customer.name[i])
        {
            flag = false;
        }
    }
...
bool Customer::operator > (const Customer &customer) const
{
    return !(operator == (customer)) && !(operator < (customer));
}
