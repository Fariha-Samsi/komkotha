...
bool PhoneNumber::operator < (const PhoneNumber &phoneNumber) const
{
    ...
    int minimumLength = min(this->phoneNumber.size(), phoneNumber.phoneNumber.size());

    for (int i = 0; i < minimumLength; i++)
    {
        if (this->phoneNumber[i] > phoneNumber.phoneNumber[i])
        {
            flag = false;
...
bool PhoneNumber::operator > (const PhoneNumber &phoneNumber) const
{
    return !(operator == (phoneNumber)) && !(operator < (phoneNumber));
}
